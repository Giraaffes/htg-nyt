...
function addReadArticleButton(row, oldArticleUrl) {
	...
	readArticleButton.appendTo($(row).find("td:eq(-1) .action-buttons-wrapper"));
...
function addEditButton(row) {
	...
	editAnchor.appendTo($(row).find("td:eq(-1) .action-buttons-wrapper"))
}

function addDeleteButton(row) {
	let deleteButton = $("<button>Slet</button>").addClass("btn delete-button");
	deleteButton.appendTo($(row).find("td:eq(-1) .action-buttons-wrapper"));
...
$("#table thead tr:eq(0) th:eq(-1)").text("Handlinger");
$("#table thead tr:eq(1) th:eq(-1)").before("<th></th><th></th>");
$("#table tbody tr").each((_, row) => {
	...
	$(row).find("td:eq(-1)").append(actionButtonsDiv);
	let oldArticleUrl = $(row).find("td:eq(-1) button.generate-link").attr("data-url");
	$(row).find("td:eq(-1) button").remove();
